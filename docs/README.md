...
## Complete Website Package — Updated June 12, 2026
...
- [x] Domain `shamliphysiotherapy.com` purchased via Hostinger
- [x] Deployed to Netlify via GitHub auto-deploy
- [x] Custom domain connected with Netlify DNS
- [x] Netlify Form email notifications configured
- [x] Registered with Google Search Console + sitemap submitted
- [x] Google Business Profile claimed
- [x] SEO keywords expanded across all 5 pages
- [x] Accessibility contrast fix (section-subtitle)
- [x] robots.txt and sitemap.xml configured and verified
...
│
├── js/
│   ├── app.js              # App logic, sliders, validation, lightboxes (~54 KB)
...
│   ├── GITHUB-DEPLOY-GUIDE.md  # GitHub Desktop push guide
...
|-----------|-------------------|----------------|
| **Hostinger** | ₹800–1,200/year | ⭐ Best for free WHOIS privacy and clean checkout |
| **GoDaddy** | ₹800–1,200/year | Popular in India, check for multi-year defaults |

> 💡 **Recommended: Hostinger** — includes free WHOIS privacy and has slightly simpler renewal management.

### Step-by-Step: Buy Domain

1. **Go to** https://www.hostinger.in or https://www.godaddy.com
2. **Search** for `shamliphysiotherapy.com`
3. **Add to cart**:
   - Ensure the duration is set to **1 Year** (or more if you wish, but watch out for default 2/3 year selections).
   - De-select any pre-ticked web hosting, website builder, or custom email options.
   - Select free basic privacy protection (decline paid "Full/Ultimate Protection").
4. **Create account** (or log in)
5. **Pay** via UPI, card, or net banking (approx. ₹800–1,200 for 1 year including tax)
...
3. Netlify will give you **nameservers** (like `dns1.p01.nsone.net`, `dns2.p01.nsone.net`, etc.)
4. Log in to your domain registrar (**Hostinger** or **GoDaddy**)
5. Go to **Domain Management** → **Nameservers** (or **DNS Nameservers**) → click **Change Nameservers**
6. Choose **Custom Nameservers** (or "I want to use my own nameservers")
7. **Paste** all 4 Netlify nameservers in order
8. **Save** changes and **Wait** 10–30 minutes (sometimes up to 48 hours) for DNS propagation
9. SSL certificate is **automatically** provisioned by Netlify (free)
...
5. Enter: `physiotherapyshamli@gmail.com`
...
- Localized SEO keywords targeting Shamli, physiotherapy, chiropractic, Dr. Jitendra Kumar Singh, Dr. Amrita Rani
...
- `robots.txt` with correct sitemap URL and crawl directives
...
### Optional / Future
1. **Bing Webmaster Tools** (optional) — https://www.bing.com/webmasters
2. **Performance optimization** — Image resizing, CLS fixes, CSS/JS minification (see implementation plan)
3. **Google Ads** (optional) — Paid search for faster visibility
